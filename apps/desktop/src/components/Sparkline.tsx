 ...
 *
 * Plain SVG, and no arithmetic: the geometry is `lib/spark.ts`, which is tested
 * without mounting anything.
 */
export function Sparkline({ points, width = 104, height = 26 }: {
  points: number[]; width?: number; height?: number;
}) {
  const spark = useSparkline(points, width, height);
    ...
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}
      ...
      <defs>
        <linearGradient id={spark.gradientId} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={spark.stroke} stopOpacity="0.28" />
          <stop offset="100%" stopColor={spark.stroke} stopOpacity="0" />
        </linearGradient>
      </defs>

      {spark.zeroY !== null && (
        <line x1="0" y1={spark.zeroY} x2={width} y2={spark.zeroY}
              stroke={spark.grid} strokeWidth="1" strokeDasharray="2 2" />
      )}

      {/* The fill is under the line and clipped to it, so the shape reads at a glance
          without the line itself getting lost in it. */}
      <polygon points={spark.area} fill={`url(#${spark.gradientId})`} />
      <polyline points={spark.line} fill="none" stroke={spark.stroke} strokeWidth="1.25"
                strokeLinejoin="round" strokeLinecap="round" />
      {/* Where it is now, which is the point the eye should land on. */}
      <circle cx={spark.dot.x} cy={spark.dot.y} r="1.9" fill={spark.stroke} />
    </svg>
  );
}
