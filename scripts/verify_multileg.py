...
`place_option_order` accepts a `legs` list, sets `order_class="mleg"` itself, and
caps out at four legs. What research cannot settle needs live paper keys:
...
Dry run by default: it fetches a chain, selects strikes, builds a vertical and an
iron condor, and prints the exact payloads without submitting anything. Pass
--submit to actually place them, which is opt-in for the obvious reason.

    python -m scripts.verify_multileg                 # inspect payloads only
    python -m scripts.verify_multileg --submit        # place, then report fills
    python -m scripts.verify_multileg --underlying QQQ --dte 30
"""
...
def parse_occ(symbol: str) -> tuple[str, date, str, Decimal] | None:
    """Split an OCC symbol into root, expiry, C/P and strike.

    HAL's `sensory/alpaca_data.parse_occ` does this properly and should be ported into
    `marketdata/`; this local copy keeps the verification script standalone so it can
    run before that port lands.
    """
    s = symbol.strip().upper()
    if len(s) < 16:
        return None
    tail = s[-15:]
    if not (tail[:6].isdigit() and tail[6] in "CP" and tail[7:].isdigit()):
        return None
    try:
        expiry = date(2000 + int(tail[:2]), int(tail[2:4]), int(tail[4:6]))
    except ValueError:
        return None
    return s[:-15], expiry, tail[6], Decimal(tail[7:]) / 1000


def contracts_from_chain(chain: Any) -> list[str]:
    """Pull option symbols out of whatever shape OptionChain actually returned.

    Written defensively on purpose: confirming this shape is one of the reasons the
    script exists, so it must survive being wrong about it and report what it saw.
    """
    if isinstance(chain, dict):
        ...
        if keys and all(parse_occ(k) for k in keys[:5]):
            return sorted(keys)
...
def pick_expiry(symbols: list[str], target_dte: int) -> date | None:
    """The listed expiry closest to the requested DTE, never nearer than 7 days."""
    today = clock.today()
    floor = today + timedelta(days=7)
    expiries = {p[1] for s in symbols if (p := parse_occ(s)) and p[1] >= floor}
    if not expiries:
        return None
...
def strikes_for(symbols: list[str], expiry: date, right: str) -> list[Decimal]:
    out = {p[3] for s in symbols if (p := parse_occ(s)) and p[1] == expiry and p[2] == right}
    return sorted(out)


def occ(root: str, expiry: date, right: str, strike: Decimal) -> str:
...
async def run(args: argparse.Namespace) -> int:
    ...
    if not symbols:
        ...
        log("           Update contracts_from_chain() -- this is exactly the unknown")
        log("           this script exists to surface.")
        return 1
    ...
    later = pick_expiry([s for s in symbols if (p := parse_occ(s)) and p[1] > expiry],
                        args.dte + 30)
    if later is None:
